...
<div id="roomAddModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <div id="roomFields">
            <div class="form-group room-field">
                <h3>객실 정보</h3>
                <%-- 객실 번호는 db에서는 auto_increment를 주기 때문에 필요 없다 --%>
                <%--            <label>객실번호</label>--%>
...
<script>
    let roomCount = 1; // 현재 객실 수를 추적하는 변수
    ...
    document.getElementById("addRoomButton").onclick = function() {
        ...
        const newRoomField = `
            <div class="form-group room-field">
                <h3>객실 정보</h3>
                <label>객실명</label>
                <input type="text" name="roomList[${roomCount - 1}].roomName" placeholder="객실명을 입력하세요" required>
                <label>객실 수량</label>
                <input type="number" name="roomList[${roomCount - 1}].roomQty" placeholder="객실 수량을 입력하세요" required min="0">
                <label>최대 인원 수</label>
                <input type="number" name="roomList[${roomCount - 1}].roomCapacity" placeholder="객실 최대 인원수를 선택해주세요" required min="0">
                <label>가격</label>
                <input type="text" name="roomList[${roomCount - 1}].roomPrice" placeholder="가격을 입력하세요" required>
                <label>할인율</label>
                <input type="text" name="roomList[${roomCount - 1}].roomDiscount" placeholder="할인율을 입력하세요" required>
                <label>상세설명</label>
                <textarea name="roomList[${roomCount - 1}].roomContents" placeholder="객실에 대한 상세정보를 입력하세요" rows="4" required></textarea>
                <label>View Type</label>
                <select name="roomList[${roomCount - 1}].roomViewType" required>
                    <option value="none">숙소의 대표적인 뷰 타입을 선택해주세요</option>
                    <option value="cityView">City View (시티뷰)</option>
                    <option value="mountainView">Mountain View (마운틴뷰)</option>
                    <option value="oceanView">Ocean View (오션뷰)</option>
        ...
        `;
        document.getElementById("roomFields").insertAdjacentHTML('beforeend', newRoomField); // 새로운 객실 필드를 DOM에 추가
    };
</script>